...
def get_grid_mesh(wave=None, teffrange=None, loggrange=None, **kwargs):
    ...
    table_grid = LinearNDInterpolator(np.array([np.log10(teffs), loggs]).T,
                                      table)
...
def get_table(teff=None, logg=None, ebv=None, vrad=None, star=None,
    ...
    if ebv is not None and ebv > 0:
        for i in range(len(mu)):
            table[:, i] = reddening.redden(
                table[:, i], wave=wave, ebv=ebv, rtype='flux', **kwargs)

    # -- that's it!
    return mu, wave, table


def get_itable(teff=None, logg=None, theta=None, mu=1, photbands=None,
...
@memoized
def _get_itable_markers(photband, gridfile, teffrange=(-np.inf, np.inf),
    ...
    for i, entries in enumerate(zip(*grid)):
        markers[i] = encode(
            **{key: entry for key, entry in zip(names, entries)})
        gridpnts[i] = entries
...
def get_limbdarkening(teff=None, logg=None, ebv=None, vrad=None, z=None,
                      photbands=None, normalised=False, **kwargs):
    """
    ...
    mus, wave, table = get_table(teff=teff, logg=logg, ebv=ebv, vrad=vrad,
                                 z=z, **kwargs)
...
def get_coeffs(teff, logg, photband, ebv=None, vrad=None, law='claret',
    ...
    @type vrad: float
    """
    mu, intensities = get_limbdarkening(teff=teff, logg=logg, ebv=ebv,
                                        vrad=vrad, photbands=[photband])
    norm_factor = intensities.max(axis=0)
    coeffs, ssr, idiff = fit_law(mu, intensities[:, 0] / norm_factor, law=law,
                                 fitmethod=fitmethod)
...
def fit_law_to_grid(photband, vrads=[0], ebvs=[0], zs=[0], law='claret',
                    fitmethod='equidist_r_leastsq', **kwargs):
    """
    ...
    for teff_, logg_ in zip(teffs, loggs):
        for ebv_, vrad_, z_ in itertools.product(ebvs, vrads, zs):
            logger.info('teff={}, logg={}, ebv={}, vrad={}, z={}'.format(
                        teff_, logg_, ebv_, vrad_, z_))
            mu, Imu = get_limbdarkening(teff=teff_, logg=logg_, ebv=ebv_,
                                        vrad=vrad_, z=z_, photbands=[photband],
                                        **kwargs)
            Imu1 = Imu.max()
            Imu = Imu[:, 0] / Imu1
            coeffs, res, dflux = fit_law(mu[mu > 0], Imu[mu > 0], law=law,
                                         fitmethod=fitmethod)
...
def generate_grid(photbands, vrads=[0], ebvs=[0], zs=[0], law='claret',
                  fitmethod='equidist_r_leastsq', outfile='mygrid.fits',
                  **kwargs):
    ...
    for photband in photbands:
        if photband in existing_bands:
            ...
            continue
        pars, coeffs, Imu1s = fit_law_to_grid(photband, vrads=vrads, ebvs=ebvs,
                                              zs=zs, **kwargs)
        cols = []
        ...
        for col in range(coeffs.shape[1]):
            cols.append(pf.Column(name='a{:d}'.format(col+1), format='E',
                                  array=coeffs[:, col]))
        ...
        newtable.header.update('SYSTEM', photband.split('.')[0],
                               'PASSBAND SYSTEM')
        newtable.header.update('FILTER', photband.split('.')[1],
                               'PASSBAND FILTER')
...
if __name__ == "__main__":
    ...
    if not sys.argv[1:]:
        ...
        # print("Calling {} with args=({}) and kwargs=({})".format(method,args,
        #                                                          kwargs)
        photbands = ['JOHNSON.U', 'JOHNSON.B', 'JOHNSON.V', 'KEPLER.V',
                     'COROT.SIS', 'COROT.EXO']
        ...
        generate_grid(['MOST.V', 'IRAC.36', 'COROT.EXO'], vrads=[0],
                      ebvs=[0.06], zs=[0, 0], law='claret',
                      fitmethod='equidist_r_leastsq', outfile='HD261903.fits')
