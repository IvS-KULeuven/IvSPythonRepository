...
from ivs.io.numpyIO import fread, fwrite
...
# Universe with Gaia"
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def findext_schlegel(ll, bb, distance=None):
  """
  Find the extinction along the line of sight, as presented in
  Schlegel et al. (1998). The procedure we use in this program
  is usefull as a first approximation, but still has some caveats
  (see later). The procedure is uses on the E(B-V) maps of Schlegel
  et al. (1998). For a certain galactic lattitude and longitude,
  we get an (interpolated) value from the E(B-V) maps and this gives
  us directly E(B-V). It is than possible to correct for the distance
  using following formulas:
  
  if |b| > 10 deg:
    E(B-V) = E(B-V)_maps * (1 - exp(-10 * r * sin(|b|)))
    where E(B-V) is the value to be used and E(B-V)_maps the value as found with
    the Schlegel dust maps.
  
  if |b| < 10 deg:
    E(B-V) = 0.032(cosec(|b|) - 1) * (1 - exp(-10 * r * sin(|b|)))
    as in Feast et al. 1990
  

  Mask values
  -----------
  The mask-map has 8 bits containing information about the processing
  history for each position on the sky.  The lowest two bits contain
  a 2-bit integer of the number of HCON's in the IRAS/ISSA plates
  (0 through 3).  The other bits are set to 1 if the following
...
def _lb2xy_schlegel(lng, lat):
  """
  Converts coordinates in lattitude and longitude to coordinates in x and y
  pixel coordinates.
  
  The x and y coordinates you find with these formulas are not the ones you can
  read from ds9, but 1 smaller. (x+1, y+1) are the coordinates you find in DS9.
  ...
  bb = lat*deg2rad
  ll = lng*deg2rad
  yy =  2048 * sqrt(1. - hs * sin(bb)) * cos(ll) + 2047.5
  xx = -2048 * hs * sqrt(1 - hs * sin(bb)) * sin(ll) + 2047.5
  return xx,yy

def _readmaps(hs, mapname='ebv'):
  """
  Read and return the data of a certain map.
  
  @param hs: hemisphere code
  @type hs: -1 or 1
  @keyword mapname : name of the map to read.
  @type mapname: float
  """
  
  if mapname.lower() == "ebv" and hs == 1:
    filename = "SFD_dust_4096_ngp.fits"
  elif mapname.lower() == "ebv" and hs == -1:
    filename = "SFD_dust_4096_sgp.fits"
  elif mapname.lower() == "mask" and hs == 1:
    filename = "SFD_mask_4096_ngp.fits"
  elif mapname.lower() == "mask" and hs == -1:
    filename = "SFD_mask_4096_sgp.fits"
  elif mapname.lower() == "i100" and hs == 1:
    filename = "SFD_i100_4096_ngp.fits"
  elif mapname.lower() == "i100" and hs == -1:
    filename = "SFD_i100_4096_sgp.fits"
    
  data   = pf.getdata(mappath + filename)
  header = pf.getheader(mappath + filename)
