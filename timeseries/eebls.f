...
c
c      integer n,nf,nb,in1,in2
c      real fmin,df,qmi,qma,bper,bpow,depth,qtran
c      dimension t(n),x(n),u(n),v(n),p(nf)
...
Cf2py intent(out) qtran
Cf2py intent(out) in1
Cf2py intent(out) in2
c     
      minbin = 5
      nbmax  = 2000
      ...
      if(fmin.lt.1.0d0/tot) fmin=1.0d0/tot
c------------------------------------------------------------------------
c
      rn=dfloat(n)
