...
class ProgressMeter(object):
    ...
    def __init__(self, **kw):
        ...
        # Does the bar has to disappear afterwards?
        self.disappear = bool(kw.get('disappear', False))
    ...
    def refresh(self, **kw):
        ...
        # Are we finished?
        if self.count >= self.total and not self.disappear:
            sys.stdout.write('\n')
        if self.count >= self.total and self.disappear:
            sys.stdout.write("Kieken")
            sys.stdout.write(self.ESC + '[2K')
            sys.stdout.write(self.ESC + '[u')
        sys.stdout.flush()
        # Timestamp
